...
3. Run in a Docker container:
    ...
    - Basic Usage:
      ...
      -a -u user-data.basic -n jammy -r
      ```
          
    - Adding static files to the ISO 
      ```bash
      docker run --rm --volume "$(pwd):/data" deserializeme/pxeless -a -u user-data.basic -n jammy -r -x /data/extras
      ```
    ...
    - Offline Installation 
      ```bash
         docker run --rm --volume "$(pwd):/data" deserializeme/pxeless -a -u user-data.basic -n jammy -r -x /data/extras -o installer-sample.sh
      ```
